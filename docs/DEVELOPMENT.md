...
视频像素层另提供独立的硬字幕/文字水印清理工具。它支持中文、英文和
中英混合文字，先在真实预览帧上自动检测或手动框选，再由 VSR 1.4.0 的
STTN 路线修复画面，最后重新挂载只读源视频的音轨。运行资源必须由用户
显式预下载，处理阶段禁止隐式联网。完整合同见
[`VIDEO_CLEANUP.md`](VIDEO_CLEANUP.md)。
